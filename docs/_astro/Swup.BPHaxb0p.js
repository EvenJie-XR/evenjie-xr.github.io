  ...
  const { once: r, ...a } = s,
    ...
    h = (m) => {
      ...
      if (f) {
        ...
        i.call(l, g), r && (l.removeEventListener(t, h, a), x(!1, l, i, u))
      }
    },
...
const I = (e, t) =>
    ...
    '',
  b = ({ hash: e } = {}) =>
    window.location.pathname + window.location.search + (e ? window.location.hash : ''),
  W = (e, t = {}) => {
    const i = p({ url: (e = e || b({ hash: !0 })), random: Math.random(), source: 'swup' }, t)
  ...
  j = (e, t, i, s) => {
    ...
    return (s = p({}, s, { signal: n.signal })), D(e, t, i, s), { destroy: () => n.abort() }
  }
...
async function B(e, t = {}) {
  ...
  const { url: f } = w.fromUrl(d),
    g = { url: f, html: m }
  return (
    !s.cache.write || (t.method && t.method !== 'GET') || e !== f || this.cache.set(g.url, g), g
  )
}
class _ {
  constructor(t) {
...
class G {
  ...
  off(t, i) {
    const s = this.get(t)
    s && i ? s.delete(i) || console.warn(`Handler for hook '${t}' not found.`) : s && s.clear()
  }
  async call(t, i, s, n) {
  ...
  parseCallArgs(t, i, s, n) {
    return i instanceof R || (typeof i != 'object' && typeof s != 'function')
      ? [i, s, n]
      : [void 0, i, s]
  }
    ...
    for (const { hook: r, handler: a, defaultHandler: l, once: c } of t)
      if (i == null || !i.done) {
        ...
        try {
          ...
          o.push(h),
            M(h) &&
              console.warn(`Swup will not await Promises in handler for synchronous hook '${r}'.`)
        } catch (h) {
          if (n) throw h
          console.error(`Error in hook '${r}':`, h)
        }
  ...
  getHandlers(t, i) {
    ...
    if (i && ((h = [{ id: 0, hook: t, handler: i }]), c)) {
      ...
      h = [{ id: 0, hook: t, handler: a[u].handler, defaultHandler: d(u) }]
    }
  ...
  sortRegistrations(t, i) {
    var s, n
    return (
      ((s = t.priority) != null ? s : 0) - ((n = i.priority) != null ? n : 0) || t.id - i.id || 0
    )
  }
  dispatchDomEvent(t, i, s) {
    if (i != null && i.done) return
    const n = { hook: t, args: s, visit: i || this.swup.visit }
    document.dispatchEvent(new CustomEvent('swup:any', { detail: n, bubbles: !0 })),
      document.dispatchEvent(new CustomEvent(`swup:${t}`, { detail: n, bubbles: !0 }))
  }
}
const J = (e) => {
...
async function X({ selector: e, elements: t }) {
    ...
    (function (o) {
      const {
        ...
        const h = window.getComputedStyle(c),
          ...
          S = Math.max(m, v),
          y = S > 0 ? (m > v ? C : A) : null
        return { type: y, timeout: S, propCount: y ? (y === C ? d.length : g.length) : 0 }
      })(o)
      return (
        !(!r || !a) &&
        new Promise((c) => {
          ...
          const m = () => {
            ...
            f = (g) => {
              g.target === o && ((performance.now() - u) / 1e3 < g.elapsedTime || (++d >= l && m()))
            }
          setTimeout(() => {
            d < l && m()
          }, a + 1),
            o.addEventListener(h, f)
        })
      )
    })(n),
  )
  s.filter(Boolean).length > 0
    ? await Promise.all(s)
    : e && console.warn(`[swup] No CSS animation duration defined on elements matching \`${e}\``)
}
function U(e, t) {
  return (e[t] || '').split(', ')
}
...
function Q(e, t = {}, i = {}) {
  ...
  if (this.shouldIgnoreVisit(e, { el: i.el, event: i.event })) return void window.location.assign(e)
...
async function Y(e, t = {}) {
  if (this.navigating) {
    if (this.visit.state >= 6)
      return (e.state = 2), void (this.onVisitEnd = () => this.performNavigation(e, t))
    ...
    if (
      (e.history.popstate ||
        (e.history.action === 'replace' || e.to.url === this.location.url
          ? k(l)
          : (this.currentHistoryIndex++, W(l, { index: this.currentHistoryIndex }))),
        ...
        (await this.hooks.call('visit:transition', e, void 0, async () => {
          ...
          e.advance(4),
            ...
            e.animation.native && document.startViewTransition
              ? await document.startViewTransition(async () => await this.renderPage(e, await a))
                  .finished
...
const Z = async function (e) {
    await this.hooks.call('animation:out:start', e, void 0, () => {
      ...
      await this.hooks.call('animation:out:await', e, { skip: !1 }, (t, { skip: i }) => {
        if (!i) return this.awaitAnimations({ selector: t.animation.selector })
      }),
  ...
  tt = function (e) {
    ...
    if (!i) return !1
    const s = ((t = i.querySelector('title')) == null ? void 0 : t.innerText) || ''
    document.title = s
      ...
      o = e.containers
        .map((r) => {
          ...
          return a && l
            ...
            : (a || console.warn(`[swup] Container missing in current document: ${r}`),
              l || console.warn(`[swup] Container missing in incoming document: ${r}`),
              !1)
        })
        .filter(Boolean)
    return (
    ...
    return (
      ...
      s &&
        ...
        (o = this.hooks.callSync(
          ...
          e,
          { options: t },
          (r, { options: a }) => (window.scrollTo(p({ top: 0, left: 0 }, a)), !0),
        )),
      o
    )
  },
  it = async function (e) {
    if (e.done) return
    const t = this.hooks.call('animation:in:await', e, { skip: !1 }, (i, { skip: s }) => {
  ...
  st = async function (e, t) {
    ...
    this.isSameResolvedUrl(b(), i) ||
      ...
      await this.hooks.call('content:scroll', e, void 0, () => this.scrollToContent(e)),
      await this.hooks.call('page:view', e, { url: this.location.url, title: document.title })
  },
  nt = function (e) {
    var t
    if (((t = e), !!t?.isSwupPlugin)) {
      if (((e.swup = this), !e._checkRequirements || e._checkRequirements()))
        return e._beforeMount && e._beforeMount(), e.mount(), this.plugins.push(e), this.plugins
...
function at(e) {
  if (typeof this.options.resolveUrl != 'function')
    return console.warn('[swup] options.resolveUrl expects a callback function.'), e
  const t = this.options.resolveUrl(e)
  return t && typeof t == 'string'
    ? t.startsWith('//') || t.startsWith('http')
      ? (console.warn('[swup] options.resolveUrl needs to return a relative url'), e)
      : t
...
class ct {
  ...
  constructor(t = {}) {
    ...
    ;(this.version = '4.7.0'),
      ...
      (this.currentHistoryIndex =
        (i = (s = window.history.state) == null ? void 0 : s.index) != null ? i : 1),
      this.enable()
  }
  async enable() {
    var t
    ...
    ;(this.clickDelegate = this.delegateEvent(i, 'click', this.handleLinkClick)),
      ...
      ((t = window.history.state) == null ? void 0 : t.source) !== 'swup' &&
        k(null, { index: this.currentHistoryIndex }),
      await q(),
  ...
  shouldIgnoreVisit(t, { el: i, event: s } = {}) {
    const { origin: n, url: o, hash: r } = w.fromUrl(t)
    return (
            ...
            n && n !== l
              ...
              : o
                ? this.hooks.callSync('link:anchor', r, { hash: o }, () => {
                    k(n + o), this.scrollToContent(r)
                  })
                : this.hooks.callSync('link:self', r, void 0, () => {
                    this.options.linkToSelf === 'navigate'
                      ? this.performNavigation(r)
                      : (k(n), this.scrollToContent(r))
                  })
        })
  }
  handlePopState(t) {
    var i, s, n, o
    const r = (i = (s = t.state) == null ? void 0 : s.url) != null ? i : window.location.href
    if (this.options.skipPopStateHandling(t) || this.isSameResolvedUrl(b(), this.location.url))
      return
    const { url: a, hash: l } = w.fromUrl(r),
    ...
    h &&
      ...
      ((c.history.direction = h - this.currentHistoryIndex > 0 ? 'forwards' : 'backwards'),
...
const ut = Object.freeze(
  Object.defineProperty({ __proto__: null, default: ct }, Symbol.toStringTag, { value: 'Module' }),
)
export { ut as S, w as l, H as m, b as n, q as w }
